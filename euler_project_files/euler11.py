"""
Time Complexity: O(r * c * n), where r = rows, c = columns, n = window size (here n=4)
...
def largest_product_in_grid(grid, n=4):
    max_product = 0
    rows = len(grid)
    cols = len(grid[0])
    ...
    for r in range(rows):
        for c in range(cols):
            if c + n <= cols:
                max_product = max(
                    max_product, product_of_elements(grid[r][c:c+n]))
            if r + n <= rows:
                max_product = max(max_product, product_of_elements(
                    [grid[r+i][c] for i in range(n)]))
            if r + n <= rows and c + n <= cols:
                max_product = max(max_product, product_of_elements(
                    [grid[r+i][c+i] for i in range(n)]))
            if r - n >= -1 and c + n <= cols:
                max_product = max(max_product, product_of_elements(
                    [grid[r-i][c+i] for i in range(n)]))
